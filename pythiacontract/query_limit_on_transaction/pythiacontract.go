...
func (s *SmartContract) NoteQuery(ctx contractapi.TransactionContextInterface, salt string) error {
	now := time.Now()
	count := 1

	checkQueryNote, _ := ctx.GetStub().GetState(salt)

	if checkQueryNote != nil {
		queryNote := new(QueryNote)
		_ = json.Unmarshal(checkQueryNote, queryNote)
	...
	err := ctx.GetStub().PutState(salt, noteBytes)

	if err != nil {
		return fmt.Errorf("Failed to note query for rate limiting purposes. %s", err.Error())
	}
...
func (s *SmartContract) Query(ctx contractapi.TransactionContextInterface, salt string, blindedHash string) (string, error) {
	...
	if checkQueryNote != nil {
		...
		timestamp := queryNote.Timestamp
		expireTime := timestamp.Add(time.Hour)
		...
		if count > QUERY_LIMIT {
			return "", fmt.Errorf("Query exceeded the rate limit for specified salt value")
		}
...
func (s *SmartContract) QueryWithProof(ctx contractapi.TransactionContextInterface, salt string, blindedHash string) (*ResultWithProof, error) {
	...
	if checkQueryNote != nil {
		...
		if count > QUERY_LIMIT {
			return nil, fmt.Errorf("Query exceeded the rate limit for specified salt value")
		}
